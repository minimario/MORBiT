...
def run_base_opt(
    ...
    for oi in range(OUT_ITER):
        ...
        if converged:
            logger.warning(
                f"[{oi+1}/{OUT_ITER}] Exiting optimization with sum(IN-DELTAs): {in_delta:.8f}, "
...
if __name__ == '__main__':
    ...
    for t, tdf in astats.groupby(['task']):
        ...
        logger.info(f'- Test      : {best_val_loss:.4f} ({best_val_obj:.4f})')
        tlist += [t]
        vlist += [best_val_obj]

    hard_tasks = np.argsort(vlist)[-3:]
    logger.info(f'Hard tasks: (avg: {np.mean(vlist):.4f})\n ALL:{np.array(vlist)}')
