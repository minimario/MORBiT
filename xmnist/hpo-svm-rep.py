    ...
    for oi in range(OUT_ITER):
        ...
        for t, tdata, tw, topt, tsch, tlambda in zip(
            ...
            for ii in range(IN_ITER):
                ...
                bX = X[bidxs]
                by = y[bidxs]
                probs = in_softmax(tw(torch.cos(outer_var(bX))))
            ...
            delta_stats += [(oi+1, t, in_delta.item(), current_lr, tlambda.item(), logC.item())]
        # print some stats
        if not batch_stats:
        ...
        if MINMAX:
            ...
            delta_stats += [(oi+1, 'SIMPLE', lambda_delta.item(), curr_lr, np.nan, logC.item())]
        ...
        if (not batch_stats) or (converged):
            ...
            for t, tdata, tw, tsched in zip(TASKS, TASK_DATA, inner_vars, in_scheds):
                ...
                tX, ty = tdata['test']
                with torch.no_grad():
                    train_out = in_softmax(tw(torch.cos(outer_var(X))))
                    train_loss = loss(train_out, y)
                    train_obj = train_loss.item()
                    train_obj += (INREG * 0.5 * wnorm(tw, INNORM)).item()
                    val_out = in_softmax(tw(torch.cos(outer_var(vX))))
                    val_loss = loss(val_out, vy)
                    test_out = in_softmax(tw(torch.cos(outer_var(tX))))
            ...
            for tt, ttdata in zip(TTASKS, TTASK_DATA):
                ...
                for eidx in range(TOBJ_MAX_EPOCHS):
                    ...
                    while start_idx < ntrain:
                        ...
                        by = y[bidxs]
                        with torch.no_grad():
                            ffX = torch.cos(outer_var(bX))
                        probs = in_softmax(iv(ffX))
                        btloss = loss(probs, by)
                ...
                with torch.no_grad():
                    ...
                    while start_idx < ntrain:
                        ...
                        bX = X[bidxs]
                        by = y[bidxs]
                        out = in_softmax(tw(torch.cos(outer_var(bX))))
