...
def run_with_step_size(lambdas=True, use_schedule=True, 
    ...
    data = {}
    if lambdas:
        learner_lambdas = Learner(sine_task, alpha=alpha, beta=beta, gamma=gamma, decay=decay, update_lambdas=True, use_schedule=use_schedule)
        ...
        # max_loss_lambdas = data["avg"]

    else:
        learner_no_lambdas = Learner(sine_task, alpha=alpha, beta=beta, gamma=gamma, update_lambdas=False, use_schedule=use_schedule)
        loss_no_lambdas, max_loss_no_lambdas = learner_no_lambdas.learn(n_outer, n_inner, n_shots)
        data["lambdas"] = False
        data["avg"] = loss_no_lambdas
        data["max"] = max_loss_no_lambdas
        # max_loss_no_lambdas = data["max_no_lambdas"]

    import glob
    pkl_files = glob.glob("runs4/*.pkl")
    run_nums = list(map(lambda x:int(x.split('_')[-1].split('.')[0]), pkl_files))
    if run_id == -1:
        if len(run_nums) == 0:
            run_id = 1
        else:
            run_id = max(run_nums)+1
    with open(f"runs4/{alpha}_{beta}_{gamma}_{lambdas}_{use_schedule}_{run_id}.pkl", "wb") as f:
        pickle.dump(data, f)
    ...
    # plt.legend()
    # plt.title("max loss")
    # plt.savefig(f"figs2/{alpha}_{beta}_{gamma}_{run_id}.png")

# run_id = 100
# for runs in range(50):
#     for alpha in [0.02]: # 0.007, 0.01, 0.013, 0.016]:
#         for beta in [0.005, 0.007, 0.009, 0.011]:
#             for gamma in [0.003]:

loss_sched = run_with_step_size(lambdas=True, use_schedule=True, 
                            alpha=0.02, beta=0.01, gamma=0.003, decay=1,
                            n_outer=6000, n_inner=3, n_shots=15, run_id=-1)
quit()            
losses_all = []
run_id = 0
for i in range(1):
    for alpha in [0.005, 0.01, 0.015, 0.02]:
        for beta in [0.002, 0.004, 0.007, 0.01]:
            for decay in [0.96, 0.92, 0.88]:
                gamma = 0.003
                for lambdas in [True, False]:
                    run_id += 1
                    print(alpha, beta, gamma, lambdas, True, run_id)
                    try:
                        a = 1
                        loss_sched = run_with_step_size(lambdas=lambdas, use_schedule=True, 
                                                    alpha=alpha, beta=beta, gamma=gamma, decay=decay,
                                                    n_outer=6000, n_inner=3, n_shots=15, run_id=-1)
                    except:
                        pass
